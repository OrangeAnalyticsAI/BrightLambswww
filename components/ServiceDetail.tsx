...
const addDarkModeClasses = (element: ReactElement, isDark: boolean): ReactElement => {
  ...
  const filteredClasses = baseClasses.filter((cls: string) => 
    !cls.startsWith('text-') || cls === 'text-white' || cls === 'text-black'
  );
...
export default function ServiceDetail({ title, content, headerContent, imageUrl = '/placeholder-service.jpg' }: ServiceDetailProps) {
  ...
  return (
    <div className={`min-h-screen py-16 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-gray-900' : 'bg-gradient-to-b from-gray-50 to-gray-100'}`}>
      <div className="max-w-6xl mx-auto">
        <div className={`rounded-2xl shadow-xl overflow-hidden transition-colors duration-300 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <div className="px-6 py-12 sm:px-12">
            <div className="flex flex-col lg:flex-row gap-12">
              ...
              <div className="lg:w-1/3">
                <div className={`rounded-lg p-4 h-full transition-colors duration-300 ${isDark ? 'bg-gray-700' : 'bg-gray-100'}`}>
                  <div className={`aspect-w-4 aspect-h-3 w-full rounded-lg overflow-hidden transition-colors duration-300 ${isDark ? 'bg-gray-600' : 'bg-gray-200'}`}>
                    <Image 
                      src={imageUrl} 
                      alt={title}
                      width={400}
                  ...
                  <div className={`mt-4 text-center text-sm transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
